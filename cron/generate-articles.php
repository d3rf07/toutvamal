...
foreach ($allItems as $item) {
    ...
    try {
        ...
        // Générer fichier statique si publié
        if ($autoPublish) {
            $articleSlug = slugify($content['title']);
            $staticPath = ARTICLES_PATH . '/' . $articleSlug . '.html';
            // Render via direct include in isolated scope
            $html = (function($s) {
                $_GET['slug'] = $s;
            ...
            if ($html && strlen($html) > 100) {
                ...
                chmod($staticPath, 0644);
                echo "  → Static HTML generated (" . strlen($html) . " bytes)\n";
            } else {
                echo "  → WARN: Static HTML generation failed\n";
            }
        }
...
// Régénérer la homepage si des articles ont été publiés
if ($generated > 0 && $autoPublish) {
    ...
    if ($homepageHtml && strlen($homepageHtml) > 100) {
        file_put_contents(ROOT_PATH . '/index.html', $homepageHtml);
        chmod(ROOT_PATH . '/index.html', 0644);
        echo "Homepage regenerated (" . strlen($homepageHtml) . " bytes)\n";
    }
}
